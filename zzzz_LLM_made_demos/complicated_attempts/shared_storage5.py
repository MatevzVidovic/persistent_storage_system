...
class SimpleMLStorage:
    ...
    def _backup_model_weights(self):
        """Backup model weights using the imported backup function"""
        if not self.model_wrapper or not self.model_wrapper.model:
            return
        
        # First save weights to a temporary location in files root
        files_folder = self.paths['files_root']
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        weights_filename = f"weights_{timestamp}.pkl"
        temp_weights_path = files_folder / weights_filename
        
        # Save weights to temporary location
        with open(temp_weights_path, 'wb') as f:
            pickle.dump(self.model_wrapper.model, f)
        
        # Use the backup function to copy to backups folder (avoiding duplicates)
        ...
        if backup_results[0] is True:
            print(f"🔄 Backed up model weights to {weights_filename}")
        elif backup_results[0] is False:
            print(f"🔄 Model weights {weights_filename} already exist in backup - skipped")
        else:
            print(f"❌ Failed to backup model weights {weights_filename}")
